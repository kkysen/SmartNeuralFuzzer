    ...
    def infer(self, graph, max_len, eos_id, k, alpha=1.0):
        ...
        for step in range(1, max_len):
            ...
            if log_prob is None:
                ...
                log_prob, pos = (
                        (out.view(batch_size, k, -1) + (log_prob * norm_old).unsqueeze(-1)) / norm_new.unsqueeze(
                    -1)).view(batch_size, -1).topk(k, dim=-1)
...
def make_model(src_vocab, tgt_vocab, N=6,
               dim_model=512, dim_ff=2048, h=H, dropout=0.1, universal=False):
    if universal:
