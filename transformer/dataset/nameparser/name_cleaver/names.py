...
class OrganizationName(Name):
    ...
    def without_extra_phrases(self):
        ...
        if "-" in name:
            ...
            if len(hyphen_parts[1]) < len(hyphen_parts[0]) and re.search(r'(\w{4,}|\s+)$',
                                                                         hyphen_parts[0]) and not re.match(
                r'^([a-zA-Z]|[0-9]+)$', hyphen_parts[1]):
...
class PoliticalMetadata(object):
    ...
    def __str__(self):
        if self.party or self.state:
            party_state = "-".join([x for x in [self.party, self.state] if
                                    x])  # because presidential candidates are listed without a state
            return str("{0} ({1})".format(str(self.name_str()), party_state))
        else:
            return str(self.name_str())
