...
class TranslationDataset(object):
    ...
    def __init__(self, path, exts, train='train', valid='valid', test='test', vocab='vocab.txt', replace_oov=None):
        ...
        self.MAX_IN_LENGTH = config.MAX_IN_LENGTH
        self.MAX_OUT_LENGTH = config.MAX_OUT_LENGTH
        strip_func_in = lambda x: x[:self.MAX_IN_LENGTH]
        strip_func_out = lambda x: x[:self.MAX_OUT_LENGTH]
        self.src_field = Field(vocab,
                               preprocessing=None,
                               postprocessing=strip_func_in)
        self.tgt_field = Field(vocab,
                               preprocessing=lambda seq: [self.INIT_TOKEN] + seq + [self.EOS_TOKEN],
                               postprocessing=strip_func_out)
    ...
    def _make_vocab(self, path, thres=0):
        word_dict = {}
    ...
    def __call__(self, graph_pool, mode='train', batch_size=32, k=1,
        ...
        for idx in order:
            ...
            if len(src_buf) == batch_size:
                if mode == 'test':
                    yield graph_pool.beam(src_buf, self.sos_id, self.MAX_OUT_LENGTH, k, device=device)
                else:
...
def get_max_len(dataset_name, in_out='out'):
    with open(f'data/{dataset_name}/train.{in_out}', 'r') as ft:
        with open(f'data/{dataset_name}/valid.{in_out}', 'r') as fv:
            with open(f'data/{dataset_name}/test.{in_out}', 'r') as fte:
                max_len = 0
                for line in ft:
                    max_len = max(len(line.split()), max_len)
                for line in fv:
                    max_len = max(len(line.split()), max_len)
                for line in fte:
                    max_len = max(len(line.split()), max_len)

    return max_len + 4  # consider 4 special characters
...
def get_dataset(dataset):
    ...
    if dataset == 'babi':
        ...
        config.MAX_OUT_LENGTH = get_max_len(dataset)
        config.MAX_IN_LENGTH = get_max_len(dataset, 'in')
        return TranslationDataset(
            'data/{}'.format(dataset),
            ('in', 'out'),
            train='train',
