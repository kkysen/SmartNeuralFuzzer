...
def run_epoch(epoch, data_iter, dev_rank, ndev, model, loss_compute, is_train=True):
    ...
    if universal:
        ...
        model.reset_stat()
    print('Epoch {} {}: Dev {} average loss: {}, accuracy {}, naive accuracy {}'.format(
        epoch, "Training" if is_train else "Evaluating",
        dev_rank, loss_compute.avg_loss, loss_compute.accuracy, loss_compute.naive_accuracy))
...
def main(dev_id, args):
    ...
    # Build graph pool
    graph_pool = GraphPool(n=config.MAX_IN_LENGTH, m=config.MAX_OUT_LENGTH)
    # Create model
