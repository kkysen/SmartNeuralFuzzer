...
class SimpleLossCompute(nn.Module):
    ...
    def naive_accuracy(self):
        return (self.n_correct_naive + self.eps) / (self.norm_term + self.eps)
    ...
    def __call__(self, y_pred, y, norm):
        ...
        n_correct_naive = np.in1d(y_pred.max(dim=-1)[1].cpu().numpy(), y.cpu().numpy())
        self.n_correct_naive += n_correct_naive.sum()
